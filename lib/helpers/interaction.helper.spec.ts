...
import { InteractionResults, KoaContextWithOIDC, Provider } from 'oidc-provider';
...
describe('InteractionHelper', () => {
  ...
  it('should call provider interactionFinished method', async () => {
    ...
    expect(provider.interactionFinished).toHaveBeenCalledWith(req, res, result, options);
  })
  ...
  it('should call provider interactionResult method', async () => {
    ...
    expect(provider.interactionResult).toHaveBeenCalledWith(req, res, result, options);
  })
})